document.addEventListener('DOMContentLoaded', function() {
  ...
  sendButton.addEventListener('click', function() {
    ...
    sendButton.textContent = '正在刷新页面...';
    statusDiv.textContent = '正在刷新选中的标签页...';
    ...
    // 先刷新页面
    chrome.tabs.reload(selectedTabId, {}, function() {
      if (chrome.runtime.lastError) {
        showError('刷新页面失败: ' + chrome.runtime.lastError.message);
        return;
      }

      statusDiv.textContent = '页面刷新中，等待加载完成...';
      ...
      const onUpdatedListener = function(tabId, changeInfo, tab) {
        if (tabId === selectedTabId && changeInfo.status === 'complete') {
          ...
          statusDiv.textContent = '页面加载完成，正在获取Cookie...';
          sendButton.textContent = '获取Cookie中...';
          
          // 页面加载完成后，获取cookie
          fetchAndSendCookies(selectedTab);
        }
      };
  ...
  // 提取获取和发送Cookie的逻辑为独立函数
  function fetchAndSendCookies(selectedTab) {
    try {
      ...
      fetch('http://127.0.0.1:8888/update/zfb/cookie', {
          method: 'POST',
          headers: {
